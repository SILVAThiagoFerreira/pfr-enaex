...
def run(config_path: Path) -> RunResult:
    ...
    blast_date, blast_time = extract_blast_datetime(sources.histo_files)

    project = load_project_frame(sources.project)
    final = load_final_frame(sources.final)
    merged = merge_frames(project, final)
    ...
    return RunResult(output_path=output_path, plan_id=plan_id, blast_date=blast_date, blast_time=blast_time, rows=len(data))
